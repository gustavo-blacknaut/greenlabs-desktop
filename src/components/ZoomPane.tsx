...
interface PropsDoZoom {
  children: ReactNode;
  /** Trocar de valor zera o zoom: a imagem anterior nao vale mais. */
  resetKey?: string | null;
}

const ZOOM_MIN = 1;
const ZOOM_MAX = 4;
const SEM_ZOOM: Zoom = { scale: ZOOM_MIN, x: 0, y: 0 };

// Zoom com a roda do mouse e arrasto para andar pela imagem, para conseguir ler
// texto pequeno na tela de quem transmite.
export default function ZoomPane({ children, resetKey }: PropsDoZoom) {
  const wrapRef = useRef<HTMLDivElement>(null);
  const dragRef = useRef<Arrasto | null>(null);
  ...
  const reset = (e: ReactMouseEvent) => { e.stopPropagation(); setZoom(SEM_ZOOM); };
